...
export const LEVELS_ALPHA = [
  ...
  // 19 — GAUNTLET (WIDE): gravity-lift to an upper run, blind ghost steps, then a rising platform
  lvl({ name: 'GAUNTLET_2', par: 3, hint: 'TIMING MATTERS', bounds: { width: 1400, height: 405 },
    spawn: { x: 56, y: 330 }, exit: { x: 1350, y: 200 },
    platforms: [floor(0, 300),
      plat(420, 200, 'solid', 150),
      plat(640, 300, 'ghost', 70), plat(800, 300, 'ghost', 70),
      { x: 960, y: 260, w: 96, h: 12, type: 'shifting', pathIndex: 0 },
      plat(1140, 200, 'solid', 240)],
    paths: [{ axis: 'v', from: { x: 960, y: 300 }, to: { x: 960, y: 200 }, speed: 80 }],
    env: [{ type: 'gravity_pulse', zone: { x: 360, y: 60, w: 56, h: 340 }, arrowDir: 'up' }] }),
  ...
  // 21 — DRIFT (WIDE): ride two moving platforms across pits; hidden spikes wait on the landings
  lvl({ name: 'DRIFT', par: 3, hint: 'RIDE AND WAIT', bounds: { width: 1340, height: 405 },
    spawn: { x: 56, y: 330 }, exit: { x: 1290, y: 390 },
    platforms: [floor(0, 250),
      { x: 300, y: 315, w: 96, h: 12, type: 'shifting', pathIndex: 0 },
      floor(600, 180),
      { x: 900, y: 300, w: 96, h: 12, type: 'shifting', pathIndex: 1 },
      floor(1180, 160)],
    paths: [{ axis: 'h', from: { x: 300, y: 315 }, to: { x: 480, y: 315 }, speed: 120 },
      { axis: 'h', from: { x: 900, y: 300 }, to: { x: 1060, y: 300 }, speed: 120 }],
    hazards: [hidden(640), hidden(1240)] }),
  ...
  // 23 — PLUNGE (WIDE): gravity-lift, then collapsing platforms under a ceiling trap to a safe ledge
  lvl({ name: 'PLUNGE', par: 3, hint: 'MIND THE CEILING AS YOU FLOAT', bounds: { width: 1280, height: 405 },
    spawn: { x: 56, y: 330 }, exit: { x: 1230, y: 170 },
    platforms: [floor(0, 320),
      plat(440, 176, 'falling', 80), plat(600, 168, 'solid', 160),
      plat(820, 176, 'falling', 80), plat(980, 170, 'solid', 280)],
    hazards: [{ x: 700, y: 114, type: 'ceiling_trap', dropDistance: 52, armProximity: 60 }, hidden(1040)],
    env: [{ type: 'gravity_pulse', zone: { x: 360, y: 60, w: 56, h: 340 }, arrowDir: 'up' }] }),
  // 24 — spiky-LOOKING safe bar over a real pit + a hidden spike + a ceiling trap on the climb
  lvl({ name: 'BARBED', par: 3, hint: 'WHAT YOU SEE IS NOT ALWAYS REAL', bounds: { width: 1340, height: 405 },
    spawn: { x: 56, y: 330 }, exit: { x: 1290, y: 390 },
    platforms: [floor(0, 200), floor(560, 220), plat(840, 316, 'solid', 80), floor(1020, 320)],
    hazards: [{ x: 230, y: 336, type: 'inverse', w: 300 }, sr(270, 374), sr(320, 374), sr(370, 374), sr(420, 374), sr(470, 374),
      { x: 760, y: 70, type: 'ceiling_trap', dropDistance: 56, armProximity: 60 }, hidden(640), hidden(900)] }),
  ...
  // 29 — NEEDLE (WIDE precision): fake/real stacks + vertical & horizontal shifting + hidden spikes
  lvl({ name: 'NEEDLE', par: 4, hint: 'PRECISION', bounds: { width: 1340, height: 405 },
    spawn: { x: 56, y: 330 }, exit: { x: 1290, y: 300 },
    platforms: [floor(0, 180),
      plat(280, 336, 'fake', 52), plat(280, 300, 'solid', 52),
      { x: 440, y: 300, w: 80, h: 12, type: 'shifting', pathIndex: 0 },
      plat(640, 300, 'solid', 60),
      plat(800, 330, 'fake', 52), plat(800, 294, 'solid', 52),
      { x: 960, y: 300, w: 80, h: 12, type: 'shifting', pathIndex: 1 },
      plat(1160, 300, 'solid', 180)],
    paths: [{ axis: 'v', from: { x: 440, y: 320 }, to: { x: 440, y: 240 }, speed: 90 },
      { axis: 'h', from: { x: 960, y: 300 }, to: { x: 1080, y: 300 }, speed: 120 }],
    hazards: [hidden(160), hidden(680)] }),
