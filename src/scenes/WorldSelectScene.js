...
export class WorldSelectScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(CONFIG.WIDTH / 2, 22, 'DÜNYA SEÇ', { fontFamily: FONT, fontSize: '16px', color: '#9fd6df' }).setOrigin(0.5);
  ...
  _card(world, cx, unlocked) {
    ...
    if (unlocked) {
      this.add.text(cx, cy - 10, `${done} / ${total} tamamlandı`, { fontFamily: FONT, fontSize: '14px', color: '#9fd6df' }).setOrigin(0.5);
      ...
      textButton(this, cx, cy + 75, '[ GİR ]', () => this.scene.start('LevelSelectScene', { world }), { size: '18px', padX: 26 });
    } else {
      // static-noise look for locked world
      this.add.text(cx, cy - 6, '[ ERİŞİM REDDEDİLDİ ]', { fontFamily: FONT, fontSize: '13px', color: '#ff5566' }).setOrigin(0.5);
      this.add.text(cx, cy + 30, "tüm SIM_ALPHA'yı tamamla", { fontFamily: FONT, fontSize: '11px', color: '#777' }).setOrigin(0.5);
      this._noise(cx, cy);
    }
  }
